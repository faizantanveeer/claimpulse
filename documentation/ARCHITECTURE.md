...
## 5. Data generation strategy: reference vs. event data

**The bug, and why it happened:** the original generator applied a single fixed seed (`Faker.seed(42)`, `random.seed(42)`) to *all five* tables uniformly. This was deliberate early on, for reproducible testing — but once the DAG began running the full pipeline repeatedly, every run regenerated byte-for-byte identical rows for every table. Since the S3 uploader (correctly) gives every file a unique timestamp+hash name, Snowflake's `COPY INTO` had no way to recognize these as "already loaded" — it saw a new filename and loaded it as new data every time. Result: raw tables multiplied on every run (1,500 claims → 3,000 → 4,500 → ...), and every `unique` test (`unique_stg_claims_claim_id`, etc.) failed on every run after the first.

**The real insight, not just the patch:** `carriers` and `providers` are **reference data** — in a real claims system, the provider/carrier roster changes slowly (a new provider onboards occasionally; the whole list doesn't get recreated daily). `claims`, `carrier_letters`, and `training_records` are **event data** — genuinely new things happening constantly, and a real pipeline expects the raw tables for these to keep growing every day. Treating both categories the same way was the actual mistake, not "using a fixed seed" in general.

**The fix, implemented in `generate_data.py`:**
- `carriers`/`providers`: generated **once**, with a fixed seed, and persisted without a date partition (`output/carriers/carriers.csv`, not `output/carriers/dt=.../...`). Every subsequent run checks whether this file already exists; if so, it loads the existing IDs instead of regenerating them, and the S3 uploader has nothing new to upload for these two tables on day 2 onward — so the raw tables genuinely stay stable at 10 and 208 rows.
- `claims`/`carrier_letters`/`training_records`: generated **fresh every run, with no fixed seed**, referencing the *existing* provider/carrier ID pool (never inventing new ones). Verified by running the generator twice on the same day: the exact same 10 carrier IDs and 208 provider IDs were reused both times, while letter counts came out genuinely different each run (2,117 vs. 2,049) — confirming real randomness on the event tables and real stability on the reference tables.
- IDs for event tables are now date-scoped (`CLM-20260819-000001`) rather than a flat sequence, so different days' batches don't collide. **Known limitation, not fixed**: running the generator manually more than once on the *same calendar day* still produces the same ID range both times (with different field values), which would create duplicate IDs in Snowflake if both same-day runs were uploaded and loaded. Not a problem for the actual `@daily`-scheduled DAG, but worth truncating RAW between manual same-day test runs.

**Why marts didn't need manual truncation to fix this, once RAW was corrected:** every staging/intermediate model is a `view` (no stored data of its own, always computed fresh at query time), and every mart is materialized as a `table`, meaning `dbt run` performs a full `CREATE OR REPLACE TABLE AS SELECT` on each run — not an append. Once RAW was truncated and reloaded cleanly, a single `dbt build` was sufficient to produce correctly de-duplicated dims and facts, with no manual intervention needed on any model downstream of RAW.

## 6. Snowflake account structure

- **RAW database** (`claimpulse_raw.raw_claims`) → loaded via scheduled `COPY INTO`, not Snowpipe (decision: keep a single scheduler — Airflow — rather than running Snowflake's own TASK scheduler alongside it; Snowpipe is a documented future upgrade once event-driven ingestion is needed). Every column is loaded as `STRING`, even dates — RAW does no casting or interpretation, that's staging's job (see section 8).
...
- **Load semantics**: `COPY INTO` uses `ON_ERROR = 'CONTINUE'` (a malformed row doesn't abort the whole file — consistent with "never silently drop data" from section 8) and relies on Snowflake's built-in 64-day load history to make repeated runs idempotent — a file that already loaded successfully is automatically skipped on the next run, which is what makes the timer-based approach safe to fire on a schedule rather than being tightly coupled to exactly when new files land.
...
## 8. Data quality resolution strategy
...
- **marts** — the final, decided version served to Looker Studio and the alerting script. Every row has a clear, defensible state by this point.
...
- **Looker Studio** (Google's free BI tool, formerly Data Studio), not Power BI as originally planned — decision made specifically for shareability: a Power BI `.pbix` file requires the desktop app or a Pro license for anyone else to view it, whereas Looker Studio produces a public, link-shareable dashboard with no viewer license needed — the right tradeoff for a portfolio piece meant to go on GitHub/LinkedIn rather than an internal company report. Connects directly to the `marts` schema via Snowflake's official Looker Studio connector. Two report pages — claims/SLA overview (from `fct_claims` + `fct_carrier_letters`), provider training compliance (from `fct_provider_training`) — both joined to `dim_providers`/`dim_carriers` for names rather than raw IDs.
...
## 11. Security and cost guardrails
